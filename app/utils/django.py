...
def handle_does_not_exist(model: type[Model], get_kwargs: dict):
    ...
    logger.info("Single model %s not found using .get with kwargs %s" % (model, get_kwargs))
    return Http404()
...
def handle_multiple_objects_returned(model: type[Model], get_kwargs: dict):
    ...
    logger.warning("Multiple models %s found during .get with kwargs %s" % (model, get_kwargs))
